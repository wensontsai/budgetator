<?php
    ...
    $stmt_expenses = $dbh->prepare("SELECT * FROM expenses WHERE user_id = :user_id ORDER BY expense_id DESC");
...
foreach($expenses_graph as $array) {
    ...
    else if ($array->expense_type === "social") {
        $social_total += $array->expense_amount;
    }
    else if ($array->expense_type === "travel") {
        $travel_total += $array->expense_amount;
    }
...
$my_final_array = array(array("label"=>"food", "value"=>$food_total),
                        ...
                        array("label"=>"social", "value"=>$social_total),
                        array("label"=>"travel", "value"=>$travel_total)
                        );
